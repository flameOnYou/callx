...
function normalizeMcpDefinition(def) {
  ...
  if (!normalized.env && normalized.environment && typeof normalized.environment === "object") {
    normalized.env = normalized.environment;
  }

  if (normalized.type === "http") {
    normalized.type = "remote";
  } else if (!normalized.type) {
    if (normalized.url && !normalized.command) {
      normalized.type = "remote";
...
function extractMcpDefinition(name, rawDef) {
  if (!rawDef || typeof rawDef !== "object" || Array.isArray(rawDef)) {
    return null;
  }

  const servers = rawDef.mcpServers;
  if (!servers || typeof servers !== "object" || Array.isArray(servers)) {
    return normalizeMcpDefinition(rawDef);
  }

  if (servers[name]) {
    return normalizeMcpDefinition(servers[name]);
  }

  const entries = Object.entries(servers).filter(([, def]) => def && typeof def === "object" && !Array.isArray(def));
  if (entries.length === 1) {
    return normalizeMcpDefinition(entries[0][1]);
  }

  console.error(`警告: MCP 服务 '${name}' 的包装配置缺少同名条目，已跳过`);
  return null;
}
...
function syncMcp(sprite) {
  ...
  for (const name of links) {
    ...
    if (!fileExists(mcpFile)) {
      console.error(`警告: MCP 服务 '${name}' 未找到配置 (${mcpFile})，已跳过`);
      continue;
    }
    const def = extractMcpDefinition(name, readJson(mcpFile, {}));
    if (!def) {
      console.error(`警告: MCP 服务 '${name}' 配置格式无效 (${mcpFile})，已跳过`);
      continue;
    }
...
function buildClaudeMcpJson(servers) {
  ...
  for (const [name, def] of Object.entries(servers)) {
    if (def.type === "remote") {
      if (!def.url) {
        console.error(`警告: MCP 服务 '${name}' 缺少 url 字段，已跳过`);
        continue;
      }
    ...
    if (!def.command) {
      console.error(`警告: MCP 服务 '${name}' 缺少 command 字段，已跳过`);
      continue;
    }
