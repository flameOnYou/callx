...
const CALL_SCRIPT = fileURLToPath(new URL("../../callx", import.meta.url));
...
function launchCall(spriteName, autoMode) {
  const args = [CALL_SCRIPT];
  if (autoMode) {
    args.push("--auto");
  }
...
function LaunchScreen({ onBack, openSprites, requestLaunch, setStatus, status }) {
  const [page, setPage] = useState("list");
  const [listIndex, setListIndex] = useState(0);
  const [actionIndex, setActionIndex] = useState(0);
  const sprites = loadSpritesDetailed();
  const hasSprites = sprites.length > 0;
  const listItems = hasSprites
    ? sprites.map((sprite) => ({
        key: sprite.name,
        label: sprite.name === runtime.getDefault() ? `${sprite.name}  [默认]` : sprite.name,
        tone: cliTone(sprite.config.cli || "opencode"),
        segments: [
          { text: sprite.config.cli || "opencode", color: cliTone(sprite.config.cli || "opencode"), bold: true },
          { text: "  ", color: theme.dim },
          { text: compactText(sprite.config.provider || "-", 16), color: sprite.config.provider ? theme.purple : theme.dim },
          { text: "  ", color: theme.dim },
          { text: compactText(sprite.config.model || "-", 20), color: theme.yellow },
        ],
      }))
    : [
        {
          key: "__empty__",
          label: "还没有可启动的智能体",
          hint: "按 Enter 去智能体页创建一个",
          tone: theme.red,
        },
      ];
  const safeListIndex = Math.min(listIndex, Math.max(0, listItems.length - 1));
  const selectedItem = listItems[safeListIndex];
  const spriteName = selectedItem?.key && selectedItem.key !== "__empty__" ? selectedItem.key : "";
  const spriteConfig = spriteName ? runtime.getSpriteConfig(spriteName) : null;
  const currentCli = spriteConfig?.cli || "opencode";
  const actionItems = [
    {
      key: "run",
      label: "普通执行",
      hint: currentCli === "claude" ? "按 Claude Code 默认权限启动" : "OpenCode 默认就是免审批",
      tone: theme.cyan,
    },
    {
      key: "auto",
      label: "自动化执行",
      hint: currentCli === "claude" ? "启动时附加 --dangerously-skip-permissions" : "OpenCode 默认就是免审批，此项等价于普通执行",
      tone: theme.orange,
    },
    {
      key: "back",
  ...
  useEffect(() => {
    setListIndex((current) => Math.min(current, Math.max(0, listItems.length - 1)));
    setActionIndex((current) => Math.min(current, Math.max(0, actionItems.length - 1)));
  }, [listItems.length, actionItems.length]);
  ...
  useInput((input, key) => {
    ...
    if (page === "list") {
      ...
      if (key.return) {
        if (!hasSprites || selectedItem.key === "__empty__") {
          setStatus({ tone: "warning", text: "还没有智能体，先去 Sprite Studio 创建一个。" });
          openSprites();
          return;
        }
        setActionIndex(0);
    ...
    if (key.upArrow || input === "k") {
      setActionIndex((current) => (current === 0 ? actionItems.length - 1 : current - 1));
      return;
    }
    if (key.downArrow || input === "j") {
      setActionIndex((current) => (current === actionItems.length - 1 ? 0 : current + 1));
      return;
    }
    if (key.return) {
      const action = actionItems[actionIndex].key;
      if (action === "back") {
        setPage("list");
        return;
      }
      const autoMode = action === "auto" && currentCli === "claude";
      requestLaunch(spriteName, autoMode);
    }
  });
  ...
  return h(
    ...
    {
      title: "callcli",
      subtitle: "Launch Runner",
      shortcuts: page === "list" ? "↑/↓ 选择智能体  Enter 下一步  Esc 返回" : "↑/↓ 选择执行方式  Enter 启动  Esc 返回列表",
      status,
    },
    page === "list"
      ? h(ListBlock, {
          title: "Sprites",
          hint: "选择一个智能体，进入启动方式页",
          items: listItems,
          selectedIndex: safeListIndex,
          active: true,
        })
      : h(ListBlock, {
          title: spriteName,
          hint: `CLI=${currentCli}  Provider=${compactText(spriteConfig?.provider || "-", 18)}  Model=${compactText(spriteConfig?.model || "-", 20)}`,
          items: actionItems,
          selectedIndex: actionIndex,
          active: true,
          visibleCount: 6,
    ...
    h(
      ...
      { marginTop: 1, flexDirection: "column" },
      page === "list"
        ? h(KeyHint, { text: hasSprites ? "Enter 进入执行方式页；这里相当于把 call 命令做成了列表启动。" : "没有智能体时会直接跳去智能体编辑页。" })
        : h(KeyHint, { text: currentCli === "claude" ? "自动化执行会给 Claude Code 追加 --dangerously-skip-permissions。" : "OpenCode 默认免审批；自动化执行和普通执行的启动效果相同。" }),
    ),
  );
}
...
function HomeScreen({ openScreen, status }) {
  ...
  const [index, setIndex] = useState(0);
  const items = [
    { label: "启动智能体", hint: "把 call / call -a 做成两步式启动页", value: "launch", tone: theme.yellow },
  ...
  return h(
    ...
    h(ListBlock, {
      title: "Main Menu",
      hint: "单面板导航",
      items: items.map((item) => ({ key: item.value, label: item.label, hint: item.hint, tone: item.tone })),
      selectedIndex: index,
      active: true,
    }),
  );
...
function App() {
  ...
  const requestLaunch = (spriteName, autoMode) => {
    pendingLaunch = { spriteName, autoMode };
    exit();
  };
  ...
  if (screen === "launch") {
    content = h(LaunchScreen, {
      onBack: goHome,
      openSprites: () => setScreen("sprites"),
      requestLaunch,
      setStatus,
      status,
    });
...
export async function runCallCli() {
  ...
  if (pendingLaunch) {
    const nextLaunch = pendingLaunch;
    pendingLaunch = null;
    launchCall(nextLaunch.spriteName, nextLaunch.autoMode);
  }
}
