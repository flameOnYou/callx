...
function providerFieldRows(providerName, provider) {
  ...
  return [
    { key: "name", label: "Provider 名", value: providerName, tone: theme.purple },
    { key: "format", label: "格式", value: provider.format || "openai", tone: theme.cyan },
    { key: "base_url", label: "API URL", value: compactText(provider.base_url || "-"), tone: theme.yellow },
    { key: "api_key", label: "API Key", value: maskSecret(provider.api_key || ""), tone: theme.orange },
    { key: "remark", label: "备注", value: compactText(provider.remark || "-"), tone: theme.text },
    {
      key: "delete",
      label: "删除 Provider",
  ...
  return h(
    ...
    ...visibleItems.map(({ item, index }) =>
      h(
        ...
        h(Text, { color: index === selectedIndex ? theme.cyan : theme.muted }, index === selectedIndex ? "› " : "  "),
        h(Text, { color: index === selectedIndex ? theme.text : theme.muted, bold: index === selectedIndex }, item.label),
        item.hint ? h(Text, { color: theme.muted }, `  ${item.hint}`) : null,
      ),
    ),
  ...
  return h(
    ...
    ...visibleRows.map((row, index) =>
      h(
        ...
        h(Text, { color: index === selectedIndex ? row.tone || theme.purple : theme.muted }, `${row.label}: `),
        h(Text, { color: index === selectedIndex ? theme.text : theme.muted, bold: index === selectedIndex }, compactText(row.value, 60)),
      ),
    ),
    ...Array.from({ length: blankCount }, (_, index) =>
      h(Text, { key: `${title}-blank-${index}`, color: theme.muted }, " "),
...
function ProviderScreen({ onBack, setStatus, status }) {
  ...
  const listItems = [{ key: "__create__", label: "＋ 新建 Provider", hint: "创建新的 API 供应商配置" }].concat(
    providers.map((provider) => ({
      key: provider.name,
      label: provider.name,
      hint: compactText(`${provider.format || "openai"}  ${provider.remark || "-"}`, 34),
    })),
  );
...
function SpriteScreen({ onBack, setStatus, status }) {
  ...
  const listItems = [{ key: "__create__", label: "＋ 新建智能体", hint: "继续保持免打字创建" }].concat(
    sprites.map((sprite) => ({
      ...
      hint: compactText(`${sprite.config.cli || "opencode"}  ${sprite.config.provider || "-"}  ${sprite.config.model || "-"}`, 44),
    })),
  );
  const safeListIndex = Math.min(listIndex, Math.max(0, listItems.length - 1));
  const selectedItem = listItems[safeListIndex];
  ...
  const rows = !spriteName || !spriteConfig
    ...
    : [
        ...
        { key: "model", label: "Model", value: compactText(spriteConfig.model || "-", 64), tone: theme.yellow },
        { key: "autoupdate", label: "Autoupdate", value: String(Boolean(spriteConfig.autoupdate)), tone: theme.green },
        {
          key: "skills",
          label: "Skills",
        ...
        { key: "default", label: "默认智能体", value: runtime.getDefault() === spriteName ? "已设为默认" : "设为默认", tone: theme.orange },
        { key: "delete", label: "删除智能体", value: "删除目录和配置", tone: theme.red },
      ];
  const safeFieldIndex = Math.min(fieldIndex, Math.max(0, rows.length - 1));
...
function WorktreeScreen({ onBack, setStatus, requestShell, status }) {
  ...
  const actions = [
    { key: "new", label: "基于当前分支新建 Worktree", hint: "自动给出 codex/ 分支名建议" },
    { key: "existing", label: "使用现有分支创建 Worktree", hint: "适合已经存在的分支" },
    { key: "list", label: "查看现有 Worktree", hint: "显示 branch -> path" },
...
function HomeScreen({ openScreen, status }) {
  ...
  const [index, setIndex] = useState(0);
  const items = [
    { label: "编辑大模型供应商", hint: "Monokai 风格的 Provider Studio", value: "providers" },
    { label: "编辑智能体", hint: "少打字、列表优先的 Sprite Studio", value: "sprites" },
    { label: "创建 Worktree", hint: "常用 git worktree 封装", value: "worktree" },
    { label: "退出", hint: "Esc 也可以直接退出", value: "exit" },
  ];
